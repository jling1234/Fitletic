...
function MenuButton({ index, onDelete }) {
  ...
  return (
    <>
        ...
        {showContextMenu && (
          <button ref={menuRef} className="delete-button" onClick={handleDelete}></button>
        )}
...
function Workoutloginpage() {
  ...
  const handleAddExercise = () => {
    setExercises([
      ...exercises,
      { id: generateUniqueId(), exerciseName: "Name : def", time: 0 },
    ]);
  };
                  ...
                  <ul className="dropdown-list">
                    {results.map((exercise) => (
                      <li
                        key={exercise.id}
                        onClick={() =>
                          handleSelectedExercise(exercise.title, exercise.id)
                        }
                      >
                        {exercise.title}
                      </li>
